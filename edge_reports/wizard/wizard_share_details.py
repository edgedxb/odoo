...
class MbkShareDetailsW(models.TransientModel):
    ...
    def print_share_details_pdf(self):
        ...
        report_name = 'Share_Details_'+ date.strftime("%y%m%d%H%M%S")
        ...
        # Report Heading
        worksheet.merge_range('A%s:K%s'%(1,1), 'SHARE DETAILS OF '+product_name+' AS ON '+ dt_string, wbf['header'])
        count += 1
        col=0
        column_width=6
        ...
        for rec in objbill:
            ...
            # Transaction Numeber
            col += 1
            worksheet.write(count, col, rec.stock_move_id.reference, wbf['content_border'])
