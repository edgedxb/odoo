...
class MbkStockSummary(models.TransientModel):
    ...
    def print_share_summary_pdf(self):
        ...
        report_name = 'Share_Summary_'+ date.strftime("%y%m%d%H%M%S")
        ...
        # Report Heading
        worksheet.merge_range('A%s:J%s'%(1,1), 'SHARE SUMMARY REPORT AS ON '+dt_strings, wbf['header'])
        ...
        col += 1
        column_width = 12
        worksheet.merge_range('I2:J2', 'Closing', wbf['content_border_bg_c'])
        worksheet.set_column(col, col, column_width)
        worksheet.write(count, col, 'Qty', wbf['content_border_bg'])
        col += 1
        column_width = 14
        ...
        for prd in objstockval.product_id:
            ...
            col+=1
            worksheet.write(count, col, opening_amt,  wbf['content_border'])
            # In Qty
            col+=1           
