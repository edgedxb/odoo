...
class MbkStockDetailsW(models.TransientModel):
    ...
    def _getdomainfilter(self):
        return [('picking_id.date_done', '<=', self.to_date),('company_id', '=', self.env.company.id),('state', '=', 'done'),
                self._get_product(), self._get_analytic(),('picking_id.picking_type_id.code','in',['incoming','outgoing'])]
    ...
    def print_cstock_summary_pdf(self):
        ...
        objbill = self.env['stock.move'].search(self._getdomainfilter())
        ...
        column_width = 18
        worksheet.set_column(col, col, column_width)
        worksheet.write(count, col, 'Transaction Number', wbf['content_border_bg'])
        col += 1
        column_width = 12
        worksheet.set_column(col, col, column_width)
        ...
        for rec in objbill:
            ...
            # Transaction Numeber
            col += 1
            worksheet.write(count, col, rec.picking_id.name, wbf['content_border'])
            
            # Date
            col += 1
            worksheet.write(count, col, rec.picking_id.date_done.strftime("%d-%m-%Y"), wbf['content_border'])
            
            # Partner
            col += 1
            if rec.picking_id.partner_id:
                worksheet.write(count, col, rec.picking_id.partner_id.name, wbf['content_border'])            
            else:
            ...
            if rec.picking_id.date_done.date()<= self.to_date:
                if rec.picking_id.picking_type_id.code=='incoming':
                    ...
                    sum_balance-=rec.product_qty               
            worksheet.write(count, col, sum_balance,  wbf['content_float_border'])
